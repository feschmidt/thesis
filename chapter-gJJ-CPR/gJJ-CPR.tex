...
We can model this dataset by solving the equation of motion of a harmonic oscillator with additional third order term of amplitude $\beta$ cf. Supplementary Material Sec.~\ref{sec:SMduffing}.
%
Fig.~\ref{fig:SMpower}
...
\begin{figure}
	...
	\caption{
		\textbf{Anharmonicity fit under assumption of constant $\kappa_i$.}
		%
		Fixing $\kappa_i$ to be the value at lowest drive power \textbf{(a)} results in significantly worse fit than introducing it as constant, but free parameter \textbf{(b)}.
		%
		However, best agreement between data and model is reached when introducing nonlinear damping, \textbf{(b)} and Fig.~\ref{fig:figure4}.
	}
	\label{fig:SMpower}
\end{figure}
...
TODO: Anharmonicity versus line width for panel c
