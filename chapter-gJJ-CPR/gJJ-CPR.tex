...
Measurements show a gate-tunable resonance frequency $f_0$ between \SIrange{7.0}{8.2}{\giga\hertz}, comparable for both devices, cf. bottom row of Fig.~\ref{fig:figure1}.
%
...
%% At first glance, any forward skew in the CPR leads to a drop in CPR slope, therefore for the same extracted $I_s^{\rm DC}$, $L_J$ should increase.
...
%
In the diffusive device, $I_s^{\rm DC}$ and $I_c^{\rm RF}$ match closely, but small deviations at the low and high end, with $I_c^{\rm RF}$ resulting in values larger than expected from DC measurements.
%
Similarly, the DC measurements of the ballistic device at \SI{1}{\kelvin} underestimate the critical current as extracted from RF for small values of $I_s^{\rm DC}$.
...
\subsection{Extraction of $I_s^{\rm DC}$ and $f_0$}
