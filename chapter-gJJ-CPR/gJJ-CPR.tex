...
\begin{align}
f_0(I_b,I_c) = \frac{f_{\lambda/2}}{2}\frac{1}{1 +  2L_J(I_b, I_c)/L_r}
\label{eq:Pogorzalek}
\end{align}
%
...
\begin{figure}
	...
	\caption{
		...
		%
		Dashed line corresponds to Josephson inductance calculated from sinusoidal CPR.
		%
...
The anharmonicity of a CPW shorted to ground by a Josephson junction is~\cite{wilsonPhotonGenerationElectromagnetic2010b,zhouHighgainWeaklyNonlinear2014}
\begin{align}
...
This corresponds to a forward skew $S=2\delta_{\rm max}/\pi -1$ of 0.03 and 0.34 for the diffusive and ballistic device, respectively, comparable to measurements from DC-measurements of the CPR~\cite{englishObservationNonsinusoidalCurrentphase2016,nandaCurrentPhaseRelationBallistic2017}.
%
...
\end{align}
%
where $\kappa=\kappa_e+\kappa_i$ and $\Delta=\omega-\omega_0$.
%
The measured $S_{11}$ is usually distorted by a setup-related microwave background of the following shape:
\begin{align}
B(\omega) = (a+b\omega+c\omega^2)e^{i(a^\prime+b^\prime\omega)},
\end{align}
%
and with additional rotation by angle $\theta$ in the complex plane, the measured $S_{11}^\prime$ looks like this:
\begin{align}
S_{11}^\prime(\omega)=B(\omega)\left(e^{i\theta}(S_{11}(\omega)+1)-1\right)
\end{align}
%
...
For measurements based on bias current sweeps, cf. Fig~\ref{fig:figure6}(a), we take the RF background at $I_b>I_s$ at which the JJ switched to the normal state and the RF resonance is removed from the measurement and divide it off the cases at which there is a circuit resonance.
...
Assuming negligible losses in the CPW, $\gamma l\approx i\beta l = i\pi\omega_0/\omega_r$ and using $\tanh(iz)=i\tan(z)$, the resonance condition of the above circuit is for the imaginary part of the admittance $Y=1/Z_q=0$:
%
\begin{align}
0 = \Im \left[ \frac{1}{i\omega_0 L_J} + \frac{1}{Z_0}\frac{Z_0+iZ_2\tan\frac{\pi\omega_0}{\omega_r}}{Z_2+iZ_0\tan\frac{\pi\omega_0}{\omega_r}}\right]
\label{eq:SolAnalytical}
\end{align}
%
The solution of this equation yields the solid line in Fig.~\ref{fig:SMval}(a).
%
We can however approximate the above by a similar method as the authors of Refs.~\cite{wallquistSelectiveCouplingSuperconducting2006a,wustmannParametricResonanceTunable2013,pogorzalekHystereticFluxResponse2017}, arriving at the expression stated in Eq.~\ref{eq:Pogorzalek} which is plotted as dashed line in Fig.~\ref{fig:SMval}(a).
%
We find that over a range of seven orders of magnitudes in $L_J$, and for the values measured, the approximation differs by no more than \SI{1}{\percent} from the analytical solution.
...
\begin{figure}
	...
	\caption{
		...
		%
		The approximation is only good regardless of $Z_J$ or $Z_0$.
	}
	\label{fig:SMval}
\end{figure}
...
\begin{align}
S_{11}(\omega)=-1-\frac{\sqrt{\kappa_e}}{S_{\rm in}}\alpha_0(\omega)\ .
\label{eq:S11anh}
\end{align}

We calculate $S_{\rm in}$ by calibrating the attenuation in our fridge, using the thermal noise power of the cryogenic HEMT as calibration source.
%
...
The resulting fit model has four parameters:
%
$\omega_0$, $\kappa_e$, $\kappa_i$ and $\beta$.
%
Using a two-step fit, we first extract one set of parameters for each data set.
...
\subsection{Internal loss rate dependence on bias current}
