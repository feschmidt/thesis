...
% 
Tape residues can be entirely removed by thermal annealing at \SI{400}{\celsius} in  an \ce{Ar}+\ce{H2} atmosphere.
% 
...
We used two different adhesive films, either polypropylene carbonate (PPC) or polybisphenol-a-carbonate (PC).
%
...
\subsubsection{Device patterning and arising challenges}
...
Cross-sectional SEM showed side-angles of approximately \SI{60}{\degree} at the contact interfaces, allowing for good overlap 
...
\begin{figure}
	...
	\caption{
		...
		% 
		\textbf{(b)} In order to make galvanic contacts to the graphene layer, the chip is placed in a \ce{CHF3}+\ce{O2} plasma, which dry-etches the BN layer.
		% 
...
The full process flow is shown in Tab.~\ref{tab:fabgJJ}.
...
\begin{tabularx}{\textwidth}{@{}cY@{}}
	...
	%\hline
	Dry etching & \ce{CHF3}+\ce{O2} \SI{40}{sccm}+\SI{4}{sccm} @ \SI{60}{\watt}, \SI{80}{\micro\bar} for \SI{1}{\minute} (etch rate roughly \SI{60}{\nano\meter\per\minute}) \\
	%\hline
	...
	\hline
	Ebeam resist & concentrated HSQ, \SI{5000}{rpm}, bake \SI{10}{\minute} at \SI{90}{\celsius} in an oven. \\
	%\hline
...
TODO: While we do not have evidence to back this up, we speculate that depositing dielectric on the entire chip area might degrade the film quality by damaging the superconducting layer.
%
We observed that ALD-deposited \ce{Al2O3} left micron-sized flakes on the chip, that could not entirely get dissolved in BOE.
%
As an alternative, we propose local deposition by liftoff, as can be done with low-temperature PECVD or ALD.
%
This could protect the surrounding circuitry and potentially enable improved circuit coherence.
