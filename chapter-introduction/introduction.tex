...
%
\begin{align}
I_\text{J}^{\rm SIS}(\delta) &= I_\text{c}\sin\delta \ ,
\end{align}
%
...
%
\begin{align}
U_\text{J}(\delta) = 1-\sum_i E_{i}^{\rm ABS} \approx E_\text{J} \frac{\delta^2}{2} - E_\text{J}\left( 1-\frac{3\sum T_i^2}{4\sum T_i} \right) \frac{\delta^4}{24} +\mathcal{O}(\delta^6)%\ , \\
...
%
In the limit of low $T_i$, i.e. for an SIS junction, the energy would be given simply by $U_\text{J}^{\rm SIS}(\delta)=1-E_\text{J}\cos(\delta)\approx E_\text{J}\delta^2/2-E_\text{J}\delta^4/24$.
%
...
\begin{align}
I(\delta) = \frac{2e}{\hbar}\frac{\partial U_\text{J}}{\partial\delta} = \frac{e\Delta}{2\hbar}\sum_i\frac{T_i\sin(\delta)}{\sqrt{1-T_i\sin^2(\delta/2)}}
\end{align}
...
%
Second, while also strongly nonlinear, the Josephson inductance of SNS junctions is significantly reduced compared to the SIS case, $L_\text{J}^{\rm SIS}=\hbar(2eI_\text{c}\cos\delta)^{-1}$.
%
...
\begin{figure}[t]
	...
	\caption{
		...
		%
		For simulating the device in QUCS, we ignored $L_\text{g}$ and $C_\text{g}$ and used the following parameters (unless swept):
		%
		$Z_0=Z_0^\prime=\SI{50}{\ohm}$, $C_\text{s}=\SI{60}{\pico\farad}$, $l=\SI{6}{\milli\meter}/\sqrt{\epsilon_\text{r}}$ with the dielectric constant of silicon $\epsilon_\text{r}=11.7$, $\alpha=1.002$,  $L_\text{J}=\SI{36}{\pico\henry}$, $C_\text{J}=\SI{1}{\femto\farad}$, $R_{\rm sg}=\SI{1}{\mega\ohm}$.
		%
		...
		%
		The Josephson junction is modeled as a network of linear lead inductance $L_\text{g}$ together with an RCSJ model of subgap-resistance $R_{\rm sg}$, junction capacitance $C_\text{J}$, nonlinear inductance $L_\text{J}$ and gate capacitance $C_\text{g}$.
		%
		...
		%
		Small $R_{\rm sg}$ corresponds to a short, large $R_{\rm sg}$ to an open to ground in parallel to $L_\text{J}$.
		%
...
%
With an added Josephson junction with inductance $L_\text{J}$ shorting the TL to ground, and neglecting $R_{\rm sg}$ and $C_{J}$, the shifted resonance frequency can be approximated by
%
\begin{align}
...
%
On the other hand, very large $R_{\rm sg}$ implies an open circuit with no losses other than the ones in the TL resonator, again approaching $Q_i$ of the bare cavity and $f_0$ shifting the the value due to the added $L_\text{J}$.
%
Intermediate resistance values suppress the resonance entirely.
%
The shift in $f_0$ for large $R_{\rm sg}$ shows the frequency shift induced by $L_\text{J}$.
